...
import com.onetoall.yjt.dialog.AlipayQRCodeGuideDialog;
import com.onetoall.yjt.dialog.AlipayQRScannerGuideDialog;
...
import com.onetoall.yjt.fragment.pay.AlipayQRCodeFragment;
...
/**
 * 支付宝收款 需要传入收款金额数key=Constants.ARG_PAY_PRICE 字段类型为String
 * Created by qinwei on 2016/10/25 14:48
 * email:qinwei_it@163.com
 */

public class QRAlipayActivity extends CaptureActivityActivity implements TabLayout.OnTabClickListener {
    ...
    protected void setContentView() {
        setContentView(R.layout.activity_qr_alipay, true);
    }
...
//        parameter.type   类型目前又qrCode决定 具体逻辑PayModel中查看
    ...
    public void showQRScannerGuideDialog() {
        AlipayQRScannerGuideDialog dialog = new AlipayQRScannerGuideDialog(this);
    ...
    public void onTabItemClick(int currentIndex, TabView.Tab tab) {
        switch (currentIndex) {
            case 0:
                UMEventUtil.onEvent(this, UMEvent.alipaysao);
                mQrAlipayGuideHintLabel.setText(R.string.mQrScannerAlipayGuideHintLabel);
                ...
                break;
            case 1:
                UMEventUtil.onEvent(this, UMEvent.alipayscode);
                mQrAlipayPriceLabel.setVisibility(View.INVISIBLE);
                mQrAlipayGuideHintLabel.setText("");
                parameter.type = PayRequestParameter.PAY_TYPE_ALIPAY;
