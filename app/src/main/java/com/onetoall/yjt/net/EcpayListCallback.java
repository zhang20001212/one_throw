...
public abstract class EcpayListCallback<T> extends AbstractCallback<T> {
    @Override
    public T convert(String content) throws AppException {
        try {
            ...
            if (code == ResponseCode.CODE_SUCCESS) {
                String tempData = jsonObject.opt("data").toString();
                String data = new JSONObject(tempData).getString("list");
